...
SYSTEM_PROMPT = (
    ...
    "3. If the platform's accessTiers list is non-empty, a line '**Additional Access:**' "
    "followed by a markdown table with columns Tier | Access Code | Approver | Description, "
    "one row per tier. These are extra, on top of the base access code, for more specific "
    "needs (e.g. dev vs. production). Never invent a tier that isn't in accessTiers.\n"
    "4. A line '**Steps:**' followed by a numbered list of the concrete actions to take "
    "(submit the request through the request method, note the approver, mention any "
    "prerequisites, then wait for provisioning; if an additional tier applies, note that it's "
    "a separate request on top of the base access).\n"
